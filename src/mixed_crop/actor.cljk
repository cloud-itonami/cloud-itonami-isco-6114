(ns mixed-crop.actor
  "MixedCropActor — the ISCO-08 6114 independent mixed-crop-growing
  actor as a `langgraph.graph/state-graph` (per ADR-2607011000 /
  CLAUDE.md Actors section). One graph run = one tend/harvest
  ...
  The unconditional invariant: the CropAdvisor can never directly
  commit a record or dispatch a robot action the MixedCropGovernor
  refuses — every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [mixed-crop.advisor :as advisor]
            [mixed-crop.governor :as governor]
            [mixed-crop.store :as store]))

(defn build-graph
  "Build a compiled MixedCropActor graph. `store` implements
  `mixed-crop.store/Store`. `advisor` implements
  `mixed-crop.advisor/Advisor` (defaults to `mock-advisor`).
